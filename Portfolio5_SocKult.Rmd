...
# Create plots for most certain effects
ggplot(random, aes(ID.Estimate.Intercept)) + geom_density()
ggplot(random, aes(ID.Estimate.Social_RepLow)) + geom_density()
ggplot(random, aes(ID.Estimate.Prog_BehGood)) + geom_density()
ggplot(random, aes(ID.Estimate.Social_RepLow.Prog_BehGood)) + geom_density()
ggplot(random, aes(ID.Estimate.Trial)) + geom_density()
ggplot(random, aes(ID.Estimate.Trial.Prog_BehGood)) + geom_density()
...
# Predicting rating
m_rating <- brm(cbind(A_rating, B_rating, C_rating, D_rating) ~ ID.Estimate.Intercept + ID.Estimate.Social_RepLow + ID.Estimate.Prog_BehGood, 
...
Further analysis for Social and Cultural dynamics exam: Are we making decisions in an optimal bayesian manner?
...
naive_prior = c(prior(normal(0,1), nlpar = "bias"))
...
expose_functions(weighted_m, vectorize = TRUE)

naive_m<-add_ic(naive_m,ic="waic")
weighted_m<-add_ic(weighted_m,ic="waic")

# Compare models 
compare_ic(m1, naive_m , ic="waic")
